...
__global__ void foo( float *A, int N, int M)
{
    ...
    if ( idx < N*M ){ //aqui idx representa a linha. Cada elemento (idx,y) e gerado
		
	A((int)(idx/M),idx%M) = idx;
...
int main( void )
{
    ...
    int block_size = 192; //maior quantidade de threads permitida em um bloco unidimensional
